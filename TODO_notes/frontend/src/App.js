...
//                console.log(cookie.get('token'))
    ...
    createTodo(text, project, users) {
        ...
        const headers = this.get_headers()
        const data = {'text': text, 'project': project, 'users': users}
        axios.post(`http://127.0.0.1:8000/api/todo/`, data, {headers})
            .then(response => {
                let new_todo = response.data
                new_todo.project = this.state.project.filter((item) => item.id === new_todo.project)[0]
                new_todo.users = this.state.users.filter((item) => item.id === new_todo.users)
                this.setState({todos: [...this.state.todo, new_todo]})
            }).catch(error => console.log(error))
    }
                        ...
                        <Switch>
                            ...
                            <Route exact path='/todo/create' component={() =>
                                <TodoForm
                                    ...
                                    createTodo={(text, project, users) => this.createTodo(text, project, users)}
                                />}
                            />
                            <Route path='/user/:id'>
